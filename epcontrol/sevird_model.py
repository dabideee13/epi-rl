...
from epcontrol.base_model import BaseModel
...
class Model(BaseModel):
    ...
    def initialize_state(self) -> np.ndarray:
        ...
        initial_state = np.zeros((self.n_districts, n_compartments, self.n_age_groups), dtype=np.float32)
        initial_state[:, Compartment.S.value, :] = self.districts_susceptibles
        initial_state[:, Compartment.E.value, :] = initial_state[:, Compartment.S.value, :] * 10 ** -6
    ...
    def stepper(
        ...
        # FIXME: make this dependent on age group
        adults_idx = 2

        #the Infected/Adults column in the SEIR state
        I_comm = seir_state[:, Compartment.E.value, adults_idx]
        ...
        for sde_step in range(sde_steps):
            ...
            sparked_districts_ags_relative_I = seir_state[sparked_districts_indices, Compartment.E.value] / np.sum(seir_state[sparked_districts_indices], axis=1)
